...
public class Network : MonoBehaviour
{
    ...
    {
        int userId = 3241241;
        SeqIdWorker idWorker = new SeqIdWorker(userId);
        ...
        UserAuthRequest userAuthRequest = new UserAuthRequest() { Id = idWorker.NextId(), Token = "test" };
        var response = await _client.SendRequestAsync(userAuthRequest);
        if (!response.IsSuccess())
        {
            Debug.Log(response);
        }
        UserJoinRequest userJoinRequest = new UserJoinRequest() { Id = idWorker.NextId(), UserId = userId };
